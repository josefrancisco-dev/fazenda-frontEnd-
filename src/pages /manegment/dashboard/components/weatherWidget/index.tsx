...
export default function WeatherWidget({ className }: WeatherWidgetProps) {
    ...
    const fetchWeather = async () => {
      try {
        const response = await fetch(
          'https://api.open-meteo.com/v1/forecast?latitude=-9.5&longitude=16.3667&current=temperature_2m,relative_humidity_2m,weather_code,wind_speed_10m&timezone=Africa%2FLuanda'
        );
  ...
  return (
    <div className={`bg-gradient-to-br from-primary/10 to-accent/10 rounded-xl p-6 shadow-sm border border-border ${className ?? ''}`}>
      <h3 className="text-sm font-semibold text-muted-foreground mb-4">Clima - Malanje</h3>
      {weather && (
